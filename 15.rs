...
fn parse_varint(data: &[u8]) -> (u64, &[u8]) {
    for i in 0..7 {
        ...
        // проверка старшего бита (если 1 - значит это конечный байт, 0 - не конечный)
        // тем самым мы доходим до последнего байта тега
        if b & 0x80 == 0 {
            // Это последний байт VARINT, преобразуем число
            // в u64 и возвращаем.
            let mut value = 0u64;
            // проходим по полученным байтам справа налево
            // после каждой итерации число сдвигаем влево на 7 и прибавляем 7 последних бит текущего байта
            // тем самым мы осовобождаемся от первых бит в 1, которые означают не конец
...
// последние три бита - это номер провода
// вс остальное - номер поля
...
fn parse_field(data: &[u8]) -> (Field, &[u8]) {
    ...
    let (fieldvalue, remainder) = match wire_type {
        WireType::Varint => {
            let (fieldvalue, remainder) = parse_varint(remainder);
            (FieldValue::Varint(fieldvalue), remainder)
        },
        WireType::Len => {
            let (fieldvalue, remainder) = parse_varint(remainder);
            (FieldValue::Len(&remainder[..fieldvalue as usize]),&remainder[fieldvalue as usize..])
        },
        //_ => todo!("Основываясь на типе проводника, создайте поле, потребляющее столько байт, сколько необходимо.")
    };
    (Field { field_num: field_num, value: fieldvalue}, remainder)
    //todo!("Верните поле и все неиспользуемые байты.")
}
...
fn parse_message<'a, T: ProtoMessage<'a>>(mut data: &'a [u8]) -> T {
    ...
    // будет столько итераций, сколько независимых сообщений в последовательности
    while !data.is_empty() {
        // parsed = (Field{ field_num: u64, value: FieldValue { Varint(u64) или Len( [u8])}} , &[u8])
